...
const PROJECT_CONFIG = {
    ...
    batchSize: 5,               // Lotes más pequeños para la prueba de concepto
    concurrencyLimit: 2,        // Concurrencia conservadora
    maxRetries: 3,
    retryDelay: 2000
};
...
function showProjectInfo() {
    ...
    console.log(`   🔄 Concurrencia: ${PROJECT_CONFIG.concurrencyLimit} lotes simultáneos`);
...
async function validatePrerequisites() {
    ...
    try {
        ...
        console.log(`   📅 Última modificación: ${fileInfo.lastModified.toLocaleString()}`);
        
        // 3. Verificar la API key de Gemini
        if (!process.env.GEMINI_API_KEY) {
            console.error('❌ Variable de entorno GEMINI_API_KEY no definida');
            console.log('💡 Tip: Crea un archivo .env con: GEMINI_API_KEY=tu_api_key');
            return false;
        }
        ...
        console.log(`🔧 Configuración: Temp=${modelInfo.config.temperature}, MaxTokens=${modelInfo.config.maxOutputTokens}`);
...
function showFinalStats(report) {
    ...
    console.log(`📝 Entradas procesadas: ${report.summary.successfulEntries}/${report.summary.totalEntries}`);
    console.log(`📦 Lotes procesados: ${report.summary.successfulBatches}/${report.summary.totalBatches}`);
    ...
    if (report.summary.failedBatches > 0) {
        ...
        console.log(`   📝 Entradas no traducidas: ${report.summary.failedEntries}`);
    ...
    if (report.summary.successfulEntries > 0) {
        ...
        console.log(`📁 Puedes revisar las traducciones en: ${PROJECT_CONFIG.outputFile}`);
    }
}
...
function showHelp() {
    ...
    console.log('📋 USO:');
    console.log('   node index.js                 # Ejecuta con configuración por defecto');
